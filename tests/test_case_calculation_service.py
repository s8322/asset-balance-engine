...
def test_asset_with_no_calculation_track_returns_empty_list() -> None:
    ...
    results = service.calculate_case(
        ...
        marriage_period=marriage_period,
        valuation_date=valuation_date,
        balance_by_asset_id={"asset-legal": 500.0},
    )

    assert results.results == []
    assert results.calculated_asset_ids == []
    assert results.skipped_asset_ids == ["asset-legal"]
    assert results.metrics.total_assets == 1
    assert results.metrics.calculated_count == 0
    assert results.metrics.skipped_count == 1
    assert results.metrics.by_asset_type == {AssetType.LEGAL_DOCUMENT: 1}
    assert (
        CalculationTrackType.LEGAL_REFERENCE in results.metrics.by_track_type
    )
...
def test_multiple_assets_mixed_types_count_all_assets_but_sum_only_calculated() -> None:
    ...
    # only calculable savings assets (pensions) contribute to savings gross maps and totals
    total_gross = sum(r.gross_shared_value for r in results.results)
    assert results.metrics.gross_shared_value_by_asset_type[AssetType.PENSION] == pytest.approx(
        total_gross
    )
...
def test_case_service_with_pension_and_investment_and_bank_accounts() -> None:
    pension_source = AssetSourceDates(first_contribution_date=date(2020, 1, 1))
    pension_asset = Asset(
        id="pension-1",
        name="Pension",
    ...
    investment_asset = Asset(
        ...
        asset_type=AssetType.INVESTMENT_ACCOUNT,
        currency=Currency.ILS,
        source_dates=AssetSourceDates(account_opened_date=date(2019, 6, 1)),
    )
    bank_asset = Asset(
        id="bank-1",
        name="Bank Account",
        asset_type=AssetType.BANK_ACCOUNT,
        currency=Currency.ILS,
        source_dates=AssetSourceDates(account_opened_date=date(2019, 1, 1)),
    )

    marriage_period = SharedPeriod(start=date(2018, 1, 1), end=date(2020, 12, 31))
    valuation_date = date(2020, 12, 31)
    balance_by_asset_id = {
        "pension-1": 1000.0,
        "investment-1": 2000.0,
    ...
    service = CaseCalculationService()
    results = service.calculate_case(
        assets=[pension_asset, investment_asset, bank_asset],
        marriage_period=marriage_period,
        valuation_date=valuation_date,
        balance_by_asset_id=balance_by_asset_id,
    )
    ...
    assert results.metrics.total_assets == 3
    assert results.metrics.by_asset_type[AssetType.PENSION] == 1
    assert results.metrics.by_asset_type[AssetType.INVESTMENT_ACCOUNT] == 1
    assert results.metrics.by_asset_type[AssetType.BANK_ACCOUNT] == 1
    assert results.metrics.by_track_type[CalculationTrackType.ACCUMULATING_SAVINGS] == 1
    assert results.metrics.by_track_type[CalculationTrackType.INVESTMENT_OR_CASH] == 2

    # gross maps include both savings and investment_or_cash tracks
    assert CalculationTrackType.INVESTMENT_OR_CASH in results.metrics.gross_shared_value_by_track_type
